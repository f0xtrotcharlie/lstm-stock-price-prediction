...
database = os.path.join(output_dir, "tickers.db")
file_path = os.path.join(output_dir, "ticker.csv")
...
def get_stock_data(symbol, start, end):
    with yfinance_lock:
        ...
        data.insert(0, "symbol", str(symbol))
...
def save_data_range(symbol, start, end, pbar=None):
    try:
        ...
        # Handle empty dates_in_db case
        if dates_in_db:
            last_date_plus1 = (datetime.strptime(dates_in_db[-1], '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d')
        else:
            last_date_plus1 = start  # If no data exists, start from the provided start date
...
def initialize_database(database_path):
    """Create the SQLite database and 'stock_data' table if it doesn't exist."""
    connection = sqlite3.connect(database_path)
    cursor = connection.cursor()

    # Drop table if it already exists (optional but ensures clean start)
